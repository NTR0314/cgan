...
# Import libs from DCGAN Tutorial (https://pytorch.org/tutorials/beginner/dcgan_faces_tutorial.html)
#%matplotlib inline
import argparse
import os
import random
...
show_images = False
save = True
saving_suffix = "testing_save_stuff"
save_path = Path(f"models/{saving_suffix}/")
...
# Print the model
with open(save_path / 'architecture.txt', 'a+') as f:
    f.write(str(netG))
...
# Load model
if not training:
    netD.load_state_dict(torch.load(model_load_path))
    netG.load_state_dict(torch.load(model_load_path))
...
#         print(f"Before generating images:")
        for i in range(num_img_inception):
...
#         print(f"Flushing GPU cache")
        ...
        if is_mean >= max(inc_scores, key= lambda x: x[0])[0]:
            # Save best
            torch.save(netD.state_dict(), save_path / f'model_weights_netD_best.pth')
            torch.save(netG.state_dict(), save_path / f'model_weights_netG_best.pth')
            no_improve_count = 0
        else:
            no_improve_count += 1
...
if save:
    np.savez(save_path / f"training_data.npz",
            img_list=img_list,
            G_losses=G_losses,
            D_losses=D_losses,
    ...
    torch.save(netD.state_dict(), save_path / f'model_weights_netD_last.pth')
    torch.save(netG.state_dict(), save_path / f'model_weights_netG_last.pth')
